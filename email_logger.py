...
def send_email(text_mesage, title_text , **kwargs):
	...
	try:
		...
		print('Email not sent to artiom.handro@gmail.com sent\n')
...
def create_train_dev_plot(dev_acc,train_acc,image_path,model_name):
	matplotlib.use('Agg')
	...
	plt.ylabel('accuracy')
	plt.xlabel('evals')
	plt.savefig(os.path.join(image_path,model_name + 'acc_results.png'))
	return os.path.join(image_path,model_name + 'acc_results.png')


def create_loss_plot(loss,image_path,image_name):
	matplotlib.use('Agg')

	line1, = plt.plot(loss, label="loss", color='tomato',linewidth=2 ,)

	plt.legend([line1], ["loss"])

	plt.ylabel('loss')
	plt.xlabel('log_batch')
	plt.savefig(os.path.join(image_path,'loss_results.png'))
	return os.path.join(image_path,'loss_results.png')
